...
- 精度が低い場合がある`,
    ...
    promptTemplate: `以下の症例を分析し、鑑別診断を3つ挙げてください。
...
- 誤った因果関係を推論するリスク`,
    ...
    scenario: `糖尿病患者の慢性合併症が進行しており、どの治療要因が病態悪化に最も影響しているかを明確にしたい。多変量の臨床データを基に、効果的な介入ポイントを特定し治療方針を策定する必要がある。`,
    promptTemplate: `以下の[患者情報]と[症例情報]をもとに、糖尿病の病態進行に影響を与えている因果関係をステップバイステップで分析してください。各因子の因果効果を明示し、どの治療介入が病態改善に最も寄与するか因果推論の視点から解説してください。

患者情報: [患者の年齢、性別、既往歴、生活習慣]
症例情報: [血糖値、HbA1c、血圧、腎機能、神経障害の有無などの臨床データ]

分析は因果グラフや因果モデルを想定し、可能な限り因果推論の手法に則って解答してください。`,
    scenario: `糖尿病患者の慢性合併症が進行しており、どの治療要因が病態悪化に最も影響しているかを明確にしたい。多変量の臨床データを基に、効果的な介入ポイントを特定し治療方針を策定する必要がある。`,
    promptTemplate: `以下の[患者情報]と[症例情報]をもとに、糖尿病の病態進行に影響を与えている因果関係をステップバイステップで分析してください。各因子の因果効果を明示し、どの治療介入が病態改善に最も寄与するか因果推論の視点から解説してください。
...
- 統合が困難な場合がある`,
    ...
    promptTemplate: `以下の[患者情報]と[症例情報]を元に、内科医、外科医、放射線科医の3つの専門家エージェントが順番に意見を述べ、それぞれの専門的視点から診断根拠、治療方針の提案、リスク評価をステップバイステップで行ってください。まず内科医が詳細な病歴と検査結果を分析し、続いて外科医が手術適応の有無を評価、最後に放射線科医が画像診断からの補足意見を述べてください。各専門家の回答は明確に区分し、最終的に3者の意見を統合した包括的な治療計画を提示してください。`,
    scenario: `高度な合併症を持つ患者が複数の診療科をまたぐ症例で入院している。内科、外科、放射線科の専門医が連携して診断と治療方針の策定を求められている状況。`,
    promptTemplate: `以下の[患者情報]と[症例情報]を元に、内科医、外科医、放射線科医の3つの専門家エージェントが順番に意見を述べ、それぞれの専門的視点から診断根拠、治療方針の提案、リスク評価をステップバイステップで行ってください。まず内科医が詳細な病歴と検査結果を分析し、続いて外科医が手術適応の有無を評価、最後に放射線科医が画像診断からの補足意見を述べてください。各専門家の回答は明確に区分し、最終的に3者の意見を統合した包括的な治療計画を提示してください。`
  },
  {
...
- 不適切な抽象化のリスク`,
    ...
    promptTemplate: `以下の[患者情報]と[症例情報]に基づき、まず臨床症状を抽象度の高いレベルで整理し、次に各症状に関連する鑑別診断を挙げてください。その後、それぞれの鑑別診断について臨床所見や検査データを詳細に分析し、可能性の高い疾患をステップバイステップで絞り込んでください。最後に、推奨される追加検査や治療方針を具体的に示してください。階層的推論を用いて、論理的かつ体系的に問題解決を進めてください。`,
    scenario: `救急外来で複雑な症例の診断を迫られている医師が、患者の多様な症状と検査結果をもとに体系的に疾患の可能性を絞り込みたい。複数の鑑別診断を階層的に評価し、最適な検査や治療方針を決定しようとしている状況。`,
    promptTemplate: `以下の[患者情報]と[症例情報]に基づき、まず臨床症状を抽象度の高いレベルで整理し、次に各症状に関連する鑑別診断を挙げてください。その後、それぞれの鑑別診断について臨床所見や検査データを詳細に分析し、可能性の高い疾患をステップバイステップで絞り込んでください。最後に、推奨される追加検査や治療方針を具体的に示してください。階層的推論を用いて、論理的かつ体系的に問題解決を進めてください。`
  },
  {
...
- 処理が重くなる`,
    ...
    promptTemplate: `以下の[患者情報]と[症例情報]をもとに、複数の制約を同時に満たす最適な治療方針をステップバイステップで検討してください。制約条件は、①薬剤の相互作用を避けること、②既往歴に基づく禁忌薬を使用しないこと、③患者の年齢や腹機能を考慮した用量調整を行うこと、④臨床検査結果に基づいた治療効果の最大化です。これらの条件を満たす治療プランの詳細と理由を論理的に説明してください。`
  },
  {
...
- 主観が入る可能性がある`,
    ...
    promptTemplate: `以下の[患者情報]と[観察データ]に基づき、仮説推論の手法を用いて最も妥当な診断を3つ挙げ、それぞれの診断を支持する根拠と除外理由を示してください。患者の年齢、既往歴、症状、血液検査結果、画像診断所見を詳細に記載してください。診断の可能性の高い順に優先順位を付けて説明してください。
...
- 最新のガイドラインが反映されない可能性`,
    ...
    promptTemplate: `以下の患者情報と症例情報に基づき、最新の診療ガイドラインに準拠した治療方針を提案してください。[患者情報]には年齢、性別、既往歴、現在の症状、検査結果を含みます。[症例情報]には現病歴、併存疾患、服薬状況を記載しています。提案する治療法の根拠として、該当する診療ガイドラインの推奨事項も明示してください。`
  },
  {
...
- すべてのカテゴリが関連するとは限らない`,
    ...
    promptTemplate: `以下の患者情報を基に、体系的な鑑別診断フレームワークを用いて鑑別診断リストを作成してください。患者情報：[患者情報]、主訴：[症例情報]、バイタルサイン：[バイタルサイン]、身体所見：[身体所見]、既往歴：[既往歴]、検査所見：[検査所見]。心血管、呼吸器、消化器、神経系など主要な系統ごとに考えられる疾患を挙げ、それぞれの可能性を簡潔に説明してください。重篤度や緊急性の高い疾患を優先的にリストアップし、次に行うべき検査や対応策も提案してください。`
  },
  {
...
- 過度に単純化される場合がある`,
    ...
    promptTemplate: `以下の[患者情報]と[検査結果]を基に、慢性心不全の悪化リスクを層別化してください。具体的には、NYHA分類、BNP値、心機能指標、既往歴（心筋梗塞、高血圧、糖尿病など）を考慮し、高リスク・中リスク・低リスクのいずれかに分類してください。それぞれのリスク群に対して推奨される治療方針や注意点も併せて提示してください。患者情報：[症例情報]`
  },
...
- 会話が長くなりすぎる場合がある`,
    ...
    promptTemplate: `以下の[患者情報]と[症例情報]に基づき、治療計画を段階的に立てます。まず、患者の主訴と既往歴から最も疑われる診断を3つ挙げてください。その後、各診断について必要な検査を提案し、検査結果を想定しながら診断の絞り込みを行います。次に、確定診断に基づき推奨される治療方針を段階的に説明してください。各ステップで内容を要約し、確認と追加情報の要求を行いながら進めてください。`
  },
  {
...
- ユーザーの負担が増える`,
    ...
    promptTemplate: `以下の[症例情報]をもとに、あなたの診断や治療方針について説明してください。その際、なぜその診断を選んだのか、どの症状や検査結果がその判断に影響したのかを具体的に考察してください。さらに、その診断に対して反証となる可能性のある所見や、他の診断候補は何かありますか？最後に、それらの診断を除外するために必要な追加検査や評価は何か、理由も含めて答えてください。`
  },
  {
...
- ユーザーの負担が増える`,
    ...
    promptTemplate: `以下の患者情報と症例情報を基に、疑問点を洗い出し、反復的に質問を投げかけて不明瞭な点を明確にしてください。まず、[患者情報]と[症例情報]を確認し、症状の発現時期、持続時間、関連症状、既往歴や服薬歴について不足している詳細情報をリストアップしてください。それぞれの疑問点について、追加で必要な具体的質問を提案し、患者からの回答を想定して情報を補完するプロセスを繰り返してください。最終的に、診断に必要な十分な情報が揃うまで続けてください。`
  },
  {
...
- 予測が難しい`,
    ...
    promptTemplate: `以下の[患者情報]と[症例情報]をもとに、現在の症状の緊急度や既往歴を考慮し、最も適切な診断候補と推奨される検査・治療方針を段階的に提示してください。状況に応じて、軽症の場合は経過観察の指示を、重症の場合は即時対応の具体策を明確に示してください。患者の年齢、性別、アレルギー歴、既存疾患も反映してください。必要に応じてリスク評価も行い、臨床ガイドラインに準拠した根拠を示してください。`
  },
  {
...
- 対話の管理が複雑`,
    ...
    promptTemplate: `以下の[患者情報]と[症例情報]を基に、医師、看護師、薬剤師、理学療法士それぞれの視点から考慮すべきポイントを挙げてください。その上で、各専門職の意見を統合し、患者にとって最適な治療計画を協調的に推論してください。特に薬物治療の相互作用、看護ケアの観点、リハビリテーションの可能性を具体的に示し、チーム医療としての連携ポイントも提案してください。`
  }
];
