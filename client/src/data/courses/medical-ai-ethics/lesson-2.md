...
- **Business Associate Agreement (BAA)**: AI開発企業と医療機関の間で、PHIの適切な取り扱いを定めた契約を締結する。
- **最小限の使用**: 目的達成に必要な最小限のPHIのみを使用する。
- **非識別化**: 可能な限り、個人を特定できない形にデータを加工する。
- **暗号化**: データの保存と転送時には暗号化を実施する。
- **アクセス制御**: PHIにアクセスできる人員を限定し、ログを記録する。
...
- **適法性、公正性、透明性**: データ処理は適法で、公正で、透明でなければならない。
- **目的の限定**: 明確で正当な目的のためにのみデータを収集し、その目的以外には使用しない。
- **データの最小化**: 目的達成に必要な最小限のデータのみを収集する。
- **正確性**: データは正確で、必要に応じて更新される。
- **保存期間の制限**: 必要な期間のみデータを保存する。
...
- **訂正権**: 不正確なデータを訂正する権利。
- **削除権（忘れられる権利）**: 一定の条件下で、自分のデータを削除させる権利。
- **処理の制限権**: データ処理を制限する権利。
...
- **HIPAA**: 500人以上に影響する侵害は、60日以内にHHSとメディアに通知。
- **GDPR**: 侵害を認識してから72時間以内に監督機関に通知。
- **個人情報保護法**: 漏洩等が発生した場合、個人情報保護委員会への報告と本人への通知。
