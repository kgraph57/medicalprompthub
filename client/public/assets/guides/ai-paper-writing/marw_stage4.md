...
-   **人間の役割**: データ収集プロセス全体を監督し、データの品質管理（QC）を行う。臨床的判断が必要なデータの解釈や入力を行う。
-   **AIの役割**: 研究プロトコルに基づき、標準化された電子データ収集フォーム（eCRF）の項目を設計する。データ入力規則（バリデーションルール）を提案し、入力ミスを減らす。
...
-   **人間の役割**: 匿名化計画を策定し、その実施を監督する。再識別リスクを評価し、所属機関のデータセキュリティポリシーを遵守する。
-   **AIの役割**: データセットに含まれる直接識別子（氏名、ID）および準識別子（郵便番号、生年月日）を特定し、k-匿名化や差分プライバシーなどの匿名化技術に基づいた具体的な処理方法を提案する。
...
-   **人間の役割**: 研究質問に答えるための適切な統計解析手法を選択する。AIが生成したコードを検証し、結果を臨床的文脈で解釈する。
-   **AIの役割**: 統計解析計画に基づき、R, Python, Stataなどの統計言語で実行可能なコードを生成する。コードには、各ステップの目的を説明する詳細なコメントを付与する。
